...
# 一年内
Fudan_ADHD <- Fudan_ADHD %>% filter(Visit_Month < 2)
# SANP total
...
# Sleep
fit <- lmer(Sleep ~ Visit_Month*Dosage_New + Age + Sex  +#疑问 使用FL2的Age还是FL3的Age
              (1|咨询号),data = Fudan_ADHD)
summary(fit)
...
Fudan_ADHD <- Fudan_ADHD %>% filter(Visit_Month > 7)
# SANP total
fit <- lmer(总分 ~ Visit_Month*Dosage_New + Age + Sex  +#疑问 使用FL2的Age还是FL3的Age
              (1|咨询号),data = Fudan_ADHD)
summary(fit)
